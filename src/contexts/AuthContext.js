...
export const AuthProvider = ({ children }) => {
    ...
    const getPremiers = async () => { //itt szedjük le külső apiból a filmeket
      try {
        const {data} = await myAxios.get('api/popular-movies')
        console.log(data)
   ...
   const getUser = async () => {
    ...
    /*const logout = async () => {
     ...
     // Komponens betöltésekor egyszer lefut
      ...
      return (
        <AuthContext.Provider value={{ loginReg, errors, getUser, user,pMovies}}>
          {children}
